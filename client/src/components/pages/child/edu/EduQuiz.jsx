...
const EduQuiz = () => {
  ...
  useEffect(() => {
    const fetchQuizResult = async () => {
      try {
        const response = await axios.get(`/children/show/quiz"`, {
  ...
  useEffect(() => {
    const fetchQuizData = async () => {
      try {
        const response = await axios.get(`/children/quiz`, {
          headers: { Authorization: `${token}` },
        });
  ...
  const sendResultsToDB = async () => {
    try {
      const scoreArray = Object.entries(scoreByCategory).map(
        ([category, score]) => ({
          category,
          score,
        })
      );
      await axios.put(`/children/update/quiz`, scoreArray, {
        headers: { Authorization: `${token}` },
      });
    } catch (error) {
    ...
    <QuizContainer>
      {showResult ? (
        <ResultContainer>
          ...
          {calculateScore() <= 40 && calculateScore() > 0 && (
            <p>도니머니와 조금 더 공부해볼까요?</p>
          )}
          {calculateScore() > 40 && calculateScore() <= 60 && (
            <p>조금만 더 공부하면 100점 가능해요!</p>
          )}
        ...
        quizData[currentCategory][currentQuestionIndex] && (
          <QuestionContainer>
            <CategoryHeader>
